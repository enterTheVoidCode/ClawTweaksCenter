...
.DESCRIPTION
    This is SEPARATE from Build-Package.ps1 (which is never touched). It:
      1. Publishes ClawTweaksCenter as a self-contained single-file win-x64 exe (no runtime needed).
      ...
      3. Zips the result into Build\ClawTweaksCenter_<version>.zip.
...
$root = Split-Path -Parent $PSScriptRoot          # repo root (ClawTweaksCenter is one level down)
...
# AssemblyName carries the version + "Setup" suffix (see ClawTweaksCenter.csproj), so the exe name
...
$outDir = Join-Path $root ("Build\ClawTweaksCenter_" + $ver)
...
# 4. Zip it.
$zip = Join-Path $root ("Build\ClawTweaksCenter_" + $ver + ".zip")
